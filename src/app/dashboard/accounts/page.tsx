...
const PLATFORMS: Record<string, { name: string; color: string; icon: string; available: boolean; description: string }> = {
  ...
  facebook: { name: "Facebook", color: "#1877F2", icon: "f", available: true, description: "Connect Facebook Pages to schedule posts and updates" },
...
export default function AccountsPage() {
  ...
  async function load() {
    ...
    if (!token) return;
    try {
      const res = await fetch(API + "/accounts", { headers: { Authorization: "Bearer " + token }, cache: "no-store" });
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400">Connect your social media accounts to schedule posts</p>
      </div>
            ...
            <div key={k} className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-5">
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  <h3 className="text-base font-semibold text-slate-900 dark:text-white">{v.name}</h3>
                  <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">{v.description}</p>

                  {connected.length > 0 && (
                    <div className="mt-3 space-y-2">
                      {connected.map((a) => (
                        <div key={a.id} className="flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-700/50 rounded-lg">
                          <CheckCircle size={16} className="text-green-500 shrink-0" />
                          <div className="flex-1 min-w-0">
                            <div className="text-sm font-medium text-slate-900 dark:text-white truncate">{a.displayName}</div>
                            <div className="text-xs text-slate-400 dark:text-slate-500">Connected</div>
                          </div>
                          <button
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {Object.entries(PLATFORMS)
          .filter(([, v]) => !v.available)
          .map(([k, v]) => (
            <div key={k} className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-4 opacity-60">
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="px-2 py-0.5 bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-400 text-[10px] font-bold rounded-full uppercase">
                      Coming Soon
                    </span>
                  </div>
                  <div className="text-xs text-slate-400 dark:text-slate-500 mt-0.5">{v.description}</div>
                </div>
      ...
      {!loading && accounts.length === 0 && (
        <div className="text-center py-8 text-slate-400 dark:text-slate-500">
          ...
          <p className="text-xs text-slate-300 dark:text-slate-600 mt-1">Click &quot;Connect with LinkedIn&quot; above to get started</p>
        </div>
      )}
    </div>
  );
