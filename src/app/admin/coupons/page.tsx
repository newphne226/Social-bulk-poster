    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">{total} total coupons</p>
        </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-4">
            <div className="flex items-center gap-3">
              ...
              <div><p className="text-xs text-gray-500 dark:text-gray-400">Total Coupons</p><p className="text-lg font-bold text-gray-900 dark:text-white">{stats.totalCoupons}</p></div>
            </div>
          </div>
          <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-4">
            <div className="flex items-center gap-3">
              ...
              <div><p className="text-xs text-gray-500 dark:text-gray-400">Active</p><p className="text-lg font-bold text-gray-900 dark:text-white">{stats.activeCoupons}</p></div>
            </div>
          </div>
          <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-4">
            <div className="flex items-center gap-3">
              ...
              <div><p className="text-xs text-gray-500 dark:text-gray-400">Total Redemptions</p><p className="text-lg font-bold text-gray-900 dark:text-white">{stats.totalRedemptions}</p></div>
            </div>
          </div>
          <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-4">
            <div className="flex items-center gap-3">
              ...
              <div><p className="text-xs text-gray-500 dark:text-gray-400">Total Discount Given</p><p className="text-lg font-bold text-gray-900 dark:text-white">{fmt(stats.totalDiscount)}</p></div>
            </div>
          </div>
        </div>
      )}
      ...
      {showForm && (
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-amber-500/30 p-6">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">{editCoupon ? "Edit Coupon" : "New Coupon"}</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <div>
              <label className="text-xs text-gray-500 block mb-1">Code *</label>
              <input type="text" value={form.code} onChange={(e) => setForm({ ...form, code: e.target.value })}
                placeholder="e.g. SAVE20" className="w-full px-3 py-2 rounded-xl border border-gray-300 dark:border-gray-800 bg-white dark:bg-gray-900 text-gray-900 dark:text-white font-mono focus:ring-2 focus:ring-amber-500" />
            </div>
            <div>
              <label className="text-xs text-gray-500 block mb-1">Name</label>
              <input type="text" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })}
                placeholder="e.g. Summer Sale" className="w-full px-3 py-2 rounded-xl border border-gray-300 dark:border-gray-800 bg-white dark:bg-gray-900 text-gray-900 dark:text-white focus:ring-2 focus:ring-amber-500" />
            </div>
            <div>
            ...
            <div>
              ...
              <input type="number" min="1" max="100" value={form.percentOff} onChange={(e) => setForm({ ...form, percentOff: e.target.value, amountOff: "" })}
                placeholder="e.g. 20" className="w-full px-3 py-2 rounded-xl border border-gray-300 dark:border-gray-800 bg-white dark:bg-gray-900 text-gray-900 dark:text-white focus:ring-2 focus:ring-amber-500" />
            </div>
            <div>
              <label className="text-xs text-gray-500 block mb-1">Discount ($)</label>
              <input type="number" min="0" step="0.01" value={form.amountOff} onChange={(e) => setForm({ ...form, amountOff: e.target.value, percentOff: "" })}
                placeholder="e.g. 10.00" className="w-full px-3 py-2 rounded-xl border border-gray-300 dark:border-gray-800 bg-white dark:bg-gray-900 text-gray-900 dark:text-white focus:ring-2 focus:ring-amber-500" />
            </div>
            <div>
            ...
            <div>
              <label className="text-xs text-gray-500 block mb-1">Max Redemptions</label>
              <input type="number" min="1" value={form.maxRedemptions} onChange={(e) => setForm({ ...form, maxRedemptions: e.target.value })}
                placeholder="Unlimited" className="w-full px-3 py-2 rounded-xl border border-gray-300 dark:border-gray-800 bg-white dark:bg-gray-900 text-gray-900 dark:text-white focus:ring-2 focus:ring-amber-500" />
            </div>
            <div>
              <label className="text-xs text-gray-500 block mb-1">Min Purchase ($)</label>
              <input type="number" min="0" step="0.01" value={form.minPurchase} onChange={(e) => setForm({ ...form, minPurchase: e.target.value })}
                placeholder="No minimum" className="w-full px-3 py-2 rounded-xl border border-gray-300 dark:border-gray-800 bg-white dark:bg-gray-900 text-gray-900 dark:text-white focus:ring-2 focus:ring-amber-500" />
            </div>
            <div>
              ...
              <input type="number" min="0" step="0.01" value={form.maxDiscount} onChange={(e) => setForm({ ...form, maxDiscount: e.target.value })}
                placeholder="No cap" className="w-full px-3 py-2 rounded-xl border border-gray-300 dark:border-gray-800 bg-white dark:bg-gray-900 text-gray-900 dark:text-white focus:ring-2 focus:ring-amber-500" />
            </div>
            <div>
            ...
            <div className="flex items-center gap-2 pt-6">
              <input type="checkbox" id="isPublic" checked={form.isPublic} onChange={(e) => setForm({ ...form, isPublic: e.target.checked })}
                className="h-4 w-4 rounded border-gray-600 bg-white dark:bg-gray-800 text-amber-500 focus:ring-amber-500" />
              <label htmlFor="isPublic" className="text-sm text-gray-600 dark:text-gray-300">Show on website</label>
            </div>
          </div>
          <div className="sm:col-span-2 lg:col-span-3 mt-4">
            <label className="text-xs text-gray-500 block mb-1">Description</label>
            <input type="text" value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })}
              placeholder="Optional description" className="w-full px-3 py-2 rounded-xl border border-gray-300 dark:border-gray-800 bg-white dark:bg-gray-900 text-gray-900 dark:text-white focus:ring-2 focus:ring-amber-500" />
          </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-12 text-gray-500 dark:text-gray-400">No coupons found</div>
      ) : (
        <div className="space-y-3">
          {coupons.map((c) => {
            ...
            return (
              <div key={c.id} className={`bg-white dark:bg-gray-900 rounded-2xl border p-5 transition-colors ${disabled ? "border-gray-300 dark:border-gray-800 opacity-60" : "border-gray-200 dark:border-gray-800 hover:border-gray-300 dark:hover:border-gray-600"}`}>
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3 mb-1">
                      <button onClick={() => copyCode(c.code)} className="flex items-center gap-1.5 px-2 py-1 rounded-lg bg-gray-100 dark:bg-gray-900 border border-gray-200 dark:border-gray-800 hover:border-amber-500 transition-colors">
                        <span className="font-mono text-sm font-bold text-gray-900 dark:text-white">{c.code}</span>
                        {copied === c.code ? <CheckCircle className="h-3.5 w-3.5 text-green-400" /> : <Copy className="h-3.5 w-3.5 text-gray-500 dark:text-gray-400" />}
                      </button>
                      {c.name && <span className="text-sm text-gray-600 dark:text-gray-300">{c.name}</span>}
                    ...
                    <div className="flex items-center gap-4 text-sm">
                      ...
                      {c.plan && <span className="text-gray-500 dark:text-gray-400">Plan: {c.plan.name}</span>}
                      {c.minPurchase && <span className="text-gray-500 dark:text-gray-400">Min: {fmt(c.minPurchase)}</span>}
                      {c.maxDiscount && <span className="text-gray-500 dark:text-gray-400">Max: {fmt(c.maxDiscount)}</span>}
                    </div>
                  ...
                  <div className="flex items-center gap-2">
                    <button onClick={() => handleToggle(c.id)}
                      className={`p-2 rounded-lg transition-colors ${c.isActive ? "text-green-400 hover:bg-green-500/10" : "text-gray-500 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-700"}`}
                    ...
                    <button onClick={() => {
                      ...
                      className="p-2 rounded-lg text-gray-500 dark:text-gray-400 hover:text-amber-400 hover:bg-amber-500/10 transition-colors">
      ...
      {/* Pagination */}
      <div className="flex items-center justify-between">
        <p className="text-sm text-gray-500 dark:text-gray-400">Showing {coupons.length === 0 ? 0 : (page - 1) * 20 + 1}–{Math.min(page * 20, total)} of {total}</p>
