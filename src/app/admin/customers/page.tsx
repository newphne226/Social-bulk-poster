          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-slate-700">
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-slate-400 uppercase">Accounts</th>
            ...
            <tbody className="divide-y divide-slate-700">
              {loading ? (
                ...
                <tr>
                  <td colSpan={8} className="px-6 py-12 text-center text-slate-400">No customers found</td>
                </tr>
              ) : (
                customers.map((c) => (
                  <tr key={c.id} className="hover:bg-slate-800/50 transition-colors cursor-pointer" onClick={() => router.push(`/admin/customers/${c.id}`)}>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                          <p className="text-sm font-medium text-white truncate">{c.name ?? "No Name"}</p>
                    ...
                    <td className="px-6 py-4">
                      <span className={`px-2 py-1 rounded-lg text-xs font-medium ${
                        c.plan === "FREE" ? "bg-slate-700 text-slate-300" :
                    ...
                    <td className="px-6 py-4 text-sm text-slate-300">{c.postsCount}</td>
                    <td className="px-6 py-4 text-sm text-slate-300">{c.accountsCount}</td>
                    <td className="px-6 py-4 text-sm text-slate-400">{formatDate(c.lastLoginAt)}</td>
                    <td className="px-6 py-4 text-sm text-slate-400">{formatDate(c.createdAt)}</td>
                    <td className="px-6 py-4">
