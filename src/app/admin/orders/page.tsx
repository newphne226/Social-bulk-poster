            ...
            <tbody className="divide-y divide-slate-700">
              {loading ? (
                ...
                orders.map((order) => {
                  ...
                  return (
                    <tr key={order.id} className="hover:bg-slate-800/50 transition-colors cursor-pointer" onClick={() => router.push(`/admin/orders/${order.id}`)}>
                      <td className="px-6 py-4">
                        <span className="text-xs text-slate-500 font-mono">#{order.id.slice(-8)}</span>
                      </td>
                      <td className="px-6 py-4">
                        <p className="text-sm text-white truncate max-w-xs">{order.caption || "No caption"}</p>
                      </td>
                      <td className="px-6 py-4">
                        <span className={`px-2 py-1 rounded-lg text-xs font-medium capitalize ${platformColors[order.platform] ?? "bg-slate-700 text-slate-300"}`}>
                          {order.platform}
                        </span>
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4 text-sm text-slate-300">{order.userName}</td>
                      <td className="px-6 py-4 text-sm text-slate-400">{order.accountName}</td>
