    ...
    <div className="space-y-6">
      ...
      {showForm && (
        <div className="bg-slate-800/50 rounded-2xl border border-amber-500/30 p-6">
          <h2 className="text-lg font-semibold text-white mb-4">{editBrand ? "Edit Brand" : "New Brand"}</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input type="url" value={form.website} onChange={(e) => setForm({ ...form, website: e.target.value })}
                placeholder="https://..." className="w-full px-3 py-2 rounded-xl border border-slate-700 bg-slate-900 text-white focus:ring-2 focus:ring-amber-500" />
            </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-12 text-slate-400">No brands yet. Create one to get started.</div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {brands.map((b) => (
            <div key={b.id} className="bg-slate-800/50 rounded-2xl border border-slate-700 p-5 hover:border-slate-600 transition-colors">
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`px-1.5 py-0.5 rounded text-[10px] ${b.isActive ? "bg-green-500/10 text-green-400" : "bg-slate-700 text-slate-400"}`}>
                      {b.isActive ? "Active" : "Inactive"}
                    </span>
                  </div>
                  <p className="text-xs text-slate-500 mt-0.5">{b._count.products} products</p>
              ...
              <div className="flex gap-2 mt-4">
                <button onClick={() => { setEditBrand(b); setForm({ name: b.name, description: b.description || "", website: b.website || "", logoUrl: b.logoUrl || "" }); setShowForm(true); }}
                  className="flex-1 px-3 py-2 rounded-xl border border-slate-700 text-slate-300 text-xs font-medium hover:bg-slate-800 transition-colors">
                  <Edit className="h-3.5 w-3.5 inline mr-1" /> Edit
                </button>
                <button onClick={() => handleDelete(b.id, b.name)}
                  className="px-3 py-2 rounded-xl border border-slate-700 text-red-400 text-xs font-medium hover:bg-red-500/10 transition-colors">
