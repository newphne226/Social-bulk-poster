...
import { Search, Ban, CheckCircle, Shield, UserMinus, AlertCircle, Edit3, X, Save, Clock, Crown, Check, ExternalLink } from "lucide-react";
...
const PLAN_COLORS: Record<string, string> = {
  ...
  SILVER: "bg-purple-500/10 text-purple-500",
                  ...
                  {loading ? (
                    ...
                    users.map((user) => (
                      <tr key={user.id} className="hover:bg-slate-800/50 transition-colors">
                        ...
                        <td className="px-6 py-4">
                          <span className={`px-2 py-1 rounded-lg text-xs font-medium ${PLAN_COLORS[user.plan] || PLAN_COLORS.FREE}`}>
                            {user.plan || "FREE"}
                          </span>
                        </td>
        ...
        <>
          ...
          {subError && (
            <div className="p-4 rounded-xl bg-red-900/20 border border-red-800 text-red-400 text-sm flex items-center gap-2">
              <AlertCircle className="h-4 w-4" />
              {subError}
              <button onClick={fetchSubRequests} className="ml-auto underline hover:text-red-300">Retry</button>
            </div>
          )}
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y divide-slate-700">
                  {subLoading ? (
                    ...
                    <tr><td colSpan={7} className="px-6 py-12 text-center text-slate-400">No subscription requests</td></tr>
                  ) : (
                    subRequests.map((req) => (
                      <tr key={req.id} className="hover:bg-slate-800/50 transition-colors">
                        ...
                        <td className="px-6 py-4">
                          <span className={`px-2 py-1 rounded-lg text-xs font-medium ${PLAN_COLORS[req.plan] || PLAN_COLORS.FREE}`}>
                            {req.plan} ({req.cycle})
                          </span>
                        </td>
                        <td className="px-6 py-4 text-sm text-white font-medium">${req.amountUsd}</td>
                        <td className="px-6 py-4">
                          <span className={`px-2 py-1 rounded-lg text-xs font-medium ${PAY_STATUS_COLORS[req.paymentStatus] || PAY_STATUS_COLORS.UNKNOWN}`}>
                            {req.paymentStatus}
                          </span>
                        </td>
                        <td className="px-6 py-4">
                          <span className={`px-2 py-1 rounded-lg text-xs font-medium ${REQ_STATUS_COLORS[req.status] || ""}`}>
                            {req.status}
                          </span>
                        </td>
                        <td className="px-6 py-4 text-sm text-slate-400">{new Date(req.createdAt).toLocaleDateString()}</td>
                        <td className="px-6 py-4 text-right">
                          {req.status === "PENDING" ? (
                            <div className="flex gap-2 justify-end">
                              <button
                                ...
                                className="px-3 py-1.5 rounded-lg bg-red-500/10 text-red-500 hover:bg-red-500/20 text-xs font-medium transition-colors disabled:opacity-50"
                              >
                                Reject
                              </button>
                              <button
                                ...
                                disabled={processingId === req.id}
                                className="px-3 py-1.5 rounded-lg bg-green-500/10 text-green-500 hover:bg-green-500/20 text-xs font-medium transition-colors disabled:opacity-50 flex items-center gap-1"
                              >
                                {processingId === req.id ? (
